...
class ReminderService {
  ...
  static Future<bool> setStatementReminder({
    ...
    try {
      ...
      for (int i = 0; i < reminderDates.length; i++) {
        ...
        final reminderKey = 'reminder_${cardId}_${period.statementDate.toIso8601String()}_$i';
  ...
  static List<DateTime> _calculateReminderDates(DateTime dueDate) {
    ...
    if (daysUntilDue > 7) {
      ...
      reminders.add(DateTime(reminder7Days.year, reminder7Days.month, reminder7Days.day, 10, 0)); // 10:00
    }
    
    // 3 gün önceden hatırlat (eğer yeterli zaman varsa)
    if (daysUntilDue > 3) {
      final reminder3Days = dueDate.subtract(const Duration(days: 3));
      reminders.add(DateTime(reminder3Days.year, reminder3Days.month, reminder3Days.day, 14, 0)); // 14:00
    }
    
    // 1 gün önceden hatırlat (eğer yeterli zaman varsa)
    if (daysUntilDue > 1) {
      final reminder1Day = dueDate.subtract(const Duration(days: 1));
      reminders.add(DateTime(reminder1Day.year, reminder1Day.month, reminder1Day.day, 18, 0)); // 18:00
    }
    
    // Son gün hatırlatıcısı (her zaman ekle)
    final lastDayReminder = DateTime(dueDate.year, dueDate.month, dueDate.day, 9, 0); // 09:00
  ...
  static String _getReminderType(int index) {
    switch (index) {
      case 0: return '7_days_before';
      case 1: return '3_days_before';
      case 2: return '1_day_before';
      case 3: return 'due_date';
      default: return 'unknown';
    }
  }
  
  /// Hatırlatıcı mesajını oluştur
  static String getReminderMessage(String type, String cardName, double amount, String dueDateText) {
    switch (type) {
      case '7_days_before':
  ...
  static Future<List<Map<String, dynamic>>> checkPendingReminders() async {
    try {
      ...
      final keys = prefs.getKeys().where((key) => key.startsWith('reminder_')).toList();
      debugPrint('🔔 ReminderService: Found ${keys.length} reminder keys');
      ...
      for (final key in keys) {
        ...
        if (reminderDataString != null) {
          try {
            // JSON parsing
            final reminderData = jsonDecode(reminderDataString) as Map<String, dynamic>;
            ...
            if (reminderData != null && !reminderData['isShown']) {
              ...
              debugPrint('🔔 Checking reminder: cardId=${reminderData['cardId']}, type=${reminderData['type']}, reminderDate=$reminderDate, dueDate=$dueDate, now=$now');
              
              // Hatırlatıcı zamanı geldi mi?
              if (now.isAfter(reminderDate) || now.isAtSameMomentAs(reminderDate)) {
                debugPrint('🔔 Adding pending reminder: ${reminderData['cardId']} - ${reminderData['type']}');
                ...
                debugPrint('🔔 Reminder not yet due: reminderDate=$reminderDate > now=$now');
              }
            } else {
              debugPrint('🔔 Skipping reminder: isShown=${reminderData?['isShown']}, data exists=${reminderData != null}');
            }
          } catch (e) {
