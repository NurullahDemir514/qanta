...
class UnifiedProviderV2 extends ChangeNotifier {
  ...
  /// Create a cash account (public method for manual creation)
    ...
    try {
      ...
      if (cashAccounts.isEmpty) {
        ...
        final cashAccount = AccountModel(
          ...
          name: 'CASH_WALLET', // Generic identifier, will be localized in UI
          balance: 0.0,
          isActive: true,
  ...
  /// Update legacy cash account names from "Cash Wallet" to "CASH_WALLET"
  Future<void> _updateLegacyCashAccountNames(List<AccountModel> cashAccounts) async {
    try {
      for (final account in cashAccounts) {
        if (account.name == 'Cash Wallet' || account.name == 'NAKİT CÜZDAN') {
          // Update the account name to generic identifier
  ...
  Future<void> checkAndResetExpiredBudgets() async {
    try {
      final now = DateTime.now();
      final budgetsToReset = <BudgetModel>[];
      final budgetsToCreate = <BudgetModel>[];
      ...
      for (final budget in _budgets) {
        ...
        switch (budget.period) {
          case BudgetPeriod.weekly:
            ...
            shouldReset = currentWeek != budget.week;
            // Calculate next week start
            final daysUntilNextWeek = (7 - now.weekday) % 7;
            nextPeriodStart = now.add(Duration(days: daysUntilNextWeek));
            break;
          case BudgetPeriod.monthly:
            shouldReset = now.month != budget.month || now.year != budget.year;
            nextPeriodStart = DateTime(now.year, now.month, 1);
            break;
          case BudgetPeriod.yearly:
            shouldReset = now.year != budget.budgetYear;
            nextPeriodStart = DateTime(now.year + 1, 1, 1);
            break;
        }

        if (shouldReset) {
          budgetsToReset.add(budget);
          
          // Create new budget for next period
          final newBudget = budget.copyWith(
            id: '', // Will be generated by Firebase
            spentAmount: 0.0,
            month: nextPeriodStart!.month,
            year: nextPeriodStart.year,
            week: budget.period == BudgetPeriod.weekly ? _getWeekOfYear(nextPeriodStart) : null,
            budgetYear: budget.period == BudgetPeriod.yearly ? nextPeriodStart.year : null,
            createdAt: DateTime.now(),
            updatedAt: DateTime.now(),
          );
      ...
      for (final budget in budgetsToReset) {
        await UnifiedBudgetService.deleteBudget(budget.id);
      }

      for (final budget in budgetsToCreate) {
        await UnifiedBudgetService.addBudget(budget);
      }

      // Reload budgets if any changes were made
      if (budgetsToReset.isNotEmpty) {
        await loadBudgets();
      }
    } catch (e) {
      debugPrint('Error checking expired budgets: $e');
    }
  }
  ...
  Future<bool> deleteBudget(String budgetId) async {
    try {
      ...
      final budgetToDelete = _budgets.firstWhere((budget) => budget.id == budgetId);
      ...
      if (!success) {
        // If deletion failed, restore the budget
      ...
      // If error occurred, restore the budget
      try {
        final budgetToRestore = _budgets.firstWhere((budget) => budget.id == budgetId);
        if (!_budgets.contains(budgetToRestore)) {
          _budgets.add(budgetToRestore);
          notifyListeners();
        }
      } catch (_) {
        // Budget not found, nothing to restore
      }
      return false;
    }
  }
  ...
  Future<AccountModel?> updateAccount({
    ...
    try {
      ...
      // Handle cash account name specially to maintain generic identifier
      String finalName = name ?? currentAccount.name;
      if (currentAccount.type == AccountType.cash && name != null) {
        // For cash accounts, always use generic identifier
        finalName = 'CASH_WALLET';
      }
  ...
  Future<bool> deleteCreditCard(String cardId) async {
    try {
      ...
      if (success) {
        ...
        await _updateSummariesLocally(); // Update summaries for balance card
        notifyListeners();
      }
      return success;
    } catch (e) {
