...
class _StatementCardState extends State<StatementCard>
  ...
  void initState() {
    super.initState();
    // Sadece dönem içi ekstre varsayılan olarak açık olsun
    _expanded = !widget.statement.isPaid && !widget.isNextStatement && widget.statement.period.startDate.isBefore(DateTime.now()) && widget.statement.period.endDate.isAfter(DateTime.now());
    ...
    // Eğer dönem içi ekstre ise, animasyonu başlat
    if (_expanded) {
      _controller.forward();
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    final totalAmount = statement
        .remainingAmount; // Use remainingAmount instead of totalWithInstallments
    final hasTransactions =
        widget.transactions.isNotEmpty || widget.installments.isNotEmpty;
              ...
              child: Column(
                ...
                children: [
                  ...
                  Row(
                    ...
                    children: [
                      Flexible(child: _buildStatementTitle()),
                      StatementBadge(
                        statement: statement,
                  ...
                  Row(
                    ...
                    children: [
                      ...
                      if (widget.showMonthlyChange &&
                          widget.previousMonthTotal != null)
                        MonthlyChangeBadge(
  ...
  Widget _buildStatementTitle() {
    ...
    if (widget.isNextStatement) {
      // Show dynamic period month instead of "Sonraki Ekstre"
      // Use consistent format with past statements (MMM yyyy)
      return Text(
        statement.period.getPeriodText(context),
        style: GoogleFonts.inter(
          fontSize: 15,
    ...
    if (statement.isPaid) {
      ...
      return Column(
        ...
        children: [
          ...
          Text(
            '${AppLocalizations.of(context)!.lastPayment}: ${statement.period.getDueDateText(context)}',
            style: GoogleFonts.inter(
              fontSize: 13,
              color: widget.isDark
  ...
  Widget _buildPeriodInfoRow() {
    ...
    if (statement.isPaid && widget.showTransactionCount) {
      ...
      return Row(
        ...
        children: [
          Flexible(
            child: Text(
              '${widget.transactions.length} ${AppLocalizations.of(context)!.transactions}',
              style: GoogleFonts.inter(
                fontSize: 13,
                color: widget.isDark
                    ? const Color(0xFF8E8E93)
    ...
    // Active/Future statement: show period, cut date and due date
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Container(
          ...
          child: Row(
            children: [
              ...
              Expanded(
                child: Row(
                  children: [
                    Icon(Icons.schedule, size: 12.sp, color: widget.isDark ? Colors.white : const Color(0xFF1C1C1E)),
                    SizedBox(width: 4.w),
                    Expanded(
                      child: Text(
                        '${AppLocalizations.of(context)!.cutOff}: ${statement.period.getEndDateText(context)}',
                        style: GoogleFonts.inter(
                          fontSize: 12.sp,
              ...
              Expanded(
                child: Row(
                  children: [
                    Icon(Icons.payments, size: 12.sp, color: widget.isDark ? Colors.white : const Color(0xFF1C1C1E)),
                    SizedBox(width: 4.w),
                    Expanded(
                      child: Text(
                        '${AppLocalizations.of(context)!.lastPayment}: ${statement.period.getDueDateText(context)}',
...
class StatementBadge extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final statusData = _getStatusData(context);
  ...
  ({String text, Color color}) _getStatusData(BuildContext context) {
    if (statement.isPaid) {
      return (
    ...
    if (statement.period.isDueSoon) {
      return (
        text:
            '${statement.period.daysUntilDue} ${AppLocalizations.of(context)!.daysLeft}',
        color: Colors.white,
      );
    }
...
class StatementTransactionsList extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final filteredTransactions = transactions
        .where((tx) => tx.installmentId == null)
        .toList();
  ...
  Widget _buildTransactionItem(
    ...
    final formattedDate = '${transactionDate.day} ${_getMonthName(transactionDate.month)}';
    ...
    return Column(
      children: [
        TransactionDesignSystem.buildTransactionItemWithIcon(
          ...
          time: formattedDate, // Add transaction date
          isDark: isDark,
  ...
  Widget _buildInstallmentItem(
    ...
    final categoryIcon = CategoryIconService.getIcon(
      installment.categoryName ?? 'other',
    );
    final categoryColor = CategoryIconService.getColorFromMap(
      installment.categoryName ?? 'other',
    );

    // Use startDate (transaction date) if available, otherwise fallback to calculated date
    final transactionDate = installment.startDate ?? 
        installment.dueDate.subtract(Duration(days: 30 * (installment.installmentNumber - 1)));
    final formattedDate = '${transactionDate.day} ${_getMonthName(transactionDate.month)}';
    ...
    return Column(
      children: [
        TransactionDesignSystem.buildTransactionItemWithIcon(
          ...
          subtitle: installment.getLocalizedDisplaySubtitle(
            AppLocalizations.of(context)!,
          ),
          amount: themeProvider.formatAmount(installment.amount),
          time: formattedDate, // Show transaction date (when it was made)
          isDark: isDark,
          specificIcon: categoryIcon,
          specificIconColor: categoryColor,
          specificBackgroundColor: categoryColor.withValues(alpha: 0.1),
...
class StatementEmptyState extends StatelessWidget {
  ...
  const StatementEmptyState({super.key, required this.isDark});
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: Center(
        child: Text(
          AppLocalizations.of(context)!.noTransactionsInStatement,
          style: GoogleFonts.inter(
            fontSize: 14,
...
class StatementLoadingState extends StatelessWidget {
  ...
  const StatementLoadingState({super.key, required this.isDark, this.message});
