-- Fix installment transaction credit card balance issue
-- The create_installment_transaction function was missing the critical step
-- of adding the total installment amount to the credit card balance

BEGIN;

-- Update create_installment_transaction to properly add total debt to credit card
...
BEGIN
    ...
    -- CRITICAL FIX: Add total debt to credit card balance
    -- This increases the debt (balance) and decreases available limit
    ...
    FOR i IN 1..p_count LOOP
        ...
        IF v_current_date <= v_today THEN
            ...
            -- Subtract the monthly payment from credit card balance
            -- This reduces the debt since the installment is already paid
